create_seq <- function (data_class){
  #Aggiorno il numero totale di eventi dopo questi vincoli
  count_event <- data.frame(data_class %>% filter(event_type=="drug purchase") 
                            %>% group_by(COD_REG) %>% count(COD_REG,name="tot_pharm_class"))
  
  data_class <- left_join(data_class, count_event, by="COD_REG")
  ...
  data_c <- as.data.frame(data_class %>% select("COD_REG","data_rif_ev","date_of_prescription","end_of_prescription",
                                          "qt_pharma","tot_pharm_class","pharm"))
  ...
  #2. Trasformo questa matrice da giornaliera a settimanale
  somm_week <- matrix(0,length(unique(codici)),52, byrow=F)
  rownames(somm_week)=unique(codici)
  nomi = NULL
  for (i in 1:52){nomi=c(nomi, paste("week_",i,sep=""))}
  colnames(somm_week) <- nomi
  
  for (i in unique(codici))
  {
    ref=1
    for (j in 1:52 ) {
      #Se in più di 3 giorni nella settimana è stato somministrato il farmaco considero che nell'inter settimana
      #è stato somministrato
      somm_week[as.character(i),j] <- ifelse(sum(somm_day[as.character(i),seq(ref,ref+6)])>3, 1, 0)
      #Passo ad analizzare la settimana successiva:
      ref = ref + 7
    }
  ...
  somm_week <- cbind("COD_REG" = unique(codici), somm_week)
  rownames(somm_week) <- seq(1:dim(somm_week)[1])
  return(somm_week)
}